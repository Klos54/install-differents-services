...
case $choise in
	...
	sed -i 56,63{'s/#//;s/fastcgi_pass 127.0.0.1:9000\;/#fastcgi_pass 127.0.0.1:9000\;/;s/With php-fpm (or other unix sockets):/#With php-fpm (or other unix sockets):/;s/With php-cgi (or other tcp sockets):/'} /etc/nginx/sites-enabled/default
